...
main(int argc, char *argv[])
{
  ...
  void SAPlace(graph_ptr device_base);
...
long long int cost(graph_ptr device) {
  ...
  //long long int temp = 20 * cost(device);
void SAPlace(graph_ptr device_base) {
  int i, j, x_old, y_old;
  float temp = 0;
...
// graph_ptr new_grid(long int len) {
//   long int i;

//   /* Allocate and declare header structure */
//   grid_ptr result = (grid_ptr) malloc(sizeof(grid));
//   if (!result) return NULL;
