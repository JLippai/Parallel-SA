...
int overlapArea(component c1, component c2) {
  ...
  int left = fmax(leftEdge(c1), leftEdge(c2));
  int right = fmin(rightEdge(c1), rightEdge(c2));
  int top = fmax(topEdge(c1), topEdge(c2));
  int bottom = fmin(bottomEdge(c1), bottomEdge(c2));
...
void SAPlace(graph_ptr device) {
  float temp = 20 * cost(device);
  float old_cost = cost(device);
...
void SAPlace_reorg(graph_ptr device_base) {
  float temp = 20 * cost(device_base);
  float old_cost = cost(device_base);
  float new_cost, rateAccept;
  int rangeX = MAX_DEVICE_WIDTH;
  int rangeY = MAX_DEVICE_HEIGHT;
  ...
  while (temp > 0.005 * old_cost / N_COMPONENTS && temp > 2) {
    for (i = 0; i < N_COMPONENTS; ++i) {
      ...
      for (j = 0; j < MOVES_PER_TEMP; ++j) {
        ...
        acceptCount += 1;
        if (rand() > 0.5*RAND_MAX)
          device_base->components[i].x = x_old + 2*(rand() - RAND_MAX/2)/RAND_MAX*rangeX;
        else
